...
Co je v MapLibre hře (`index.html`) hotové:
  ...
  - **FX overlay** (`#fx` canvas): NPC + props sprity, hranolky/smažák (PROCEDUÁLNÍ kresba, ne emoji — `drawFries`/`drawSmaz`), splat particly — vše přes `map.project([lng,lat])`. Vše ve světě v lng/lat, převody přes `MLAT`/`mlng`/`moveGeo`/`distM`.
  - **PEVNÉ velikosti** (žádné škálování dle vzdálenosti — David to chtěl): `HUMAN_H` (NPC+Šimmy), `PROP_PX` (mobiliář), `FRY_PX`/`SMAZ_PX`. Šimmy DOM 25×54, zoom 19.9, pitch 62. Ladí se jedním číslem.
  - **Props** (zeleň+mobiliář): `scatterProps()` + `surfaceAt()` (přes `queryRenderedFeatures`) → keře/stromy/kytky na zeleň, lampy/koše/popelnice/zastávky mimo silnici. Sprity `assets/props/strom_clean.png` + `assets/props/gen/{ker,lampa,zastavka,kytky,kos,popelnice}.png`.
  - **Menu + UI** (port ze starého enginu, ale nově hezké): stavový automat `state` = MENU/PLAYING/OVER. Titulka (`#menu`, na pozadí orbituje kamera), nastavení (hudba/zvuky on+vol), žebříček TOP SMAŽKY (Firebase RTDB `DB_URL`, sdílený se starým enginem), nick, game-over obrazovka. Styl: Oswald font, zlato (#ffd23f) + tmavá navy, `.overlay`/`.panel`/`.mbtn`.
  - **Audio**: hudba `assets/music.mp3` (přes GainNode kvůli iOS hlasitosti) + SFX `assets/sfx/*.wav` (WebAudio, `sfx(name,vol)`), odemknutí na první dotyk. Persistence v localStorage (`smazak_musicOn/sfxOn/musicVol/sfxVol`).
  - Atribuce OSM (povinná) schovaná do kompaktního ⓘ vlevo dole.
  - **TODO dál**: okna na barácích (potřebuje vlastní WebGL/Three vrstvu — MapLibre neumí texturu na stěny extruzí), POI cedule reálných obchodů, sběr párna/perky, mobilní VK auto-zoom.
